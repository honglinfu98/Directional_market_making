...
\paragraph{Why the decoupling is sound: two one-way valves.} Three exact facts,
none approximate. (i) \emph{Factorization is an identity}: any MTPP satisfies
$\lambda_k=\Lambda\cdot(\lambda_k/\Lambda)$; Eq.~\eqref{eq:factorization} is the
form in which Chang, Boyd \& Smyth (AISTATS 2024) parameterize set-valued
MTPPs (the lineage of this codebase), so adopting it loses no generality.
(ii) \emph{The likelihood separates additively}:
$\ell=\big[\sum_i\log\Lambda(t_i)-\int\Lambda\big]
+\sum_i\log p(k_i\mid u(t_i^-))
=\ell_{\mathrm{time}}(\theta_g)+\ell_{\mathrm{mark}}(\theta_m)$
with \emph{disjoint} parameter blocks, so block-wise estimation --- moments
(Kirchner) for $\theta_g$, MLE for $\theta_m$ --- \emph{is} the joint MLE
restricted to each block; and since $\ell_{\mathrm{mark}}$ is teacher-forced on
real times and contains no ground parameter, the fitted ground may be
transplanted after mark training. (iii) \emph{The count process is autonomous}:
because the accumulators are mark-blind ($+1$ per event regardless of channel),
$\Lambda$ is a function of event times only --- the times form a self-contained
scalar Hawkes and marks are a posterior coloring of its ticks. Coloring ticks
cannot change their number, which upgrades the lemma to a theorem about
\emph{simulation}: the entire Fano curve is a set-once constant of
$(a,\beta,n)$. In valve language: rate-neutrality closes marks$\to$rate (the
training decouples); mark-blindness closes rate$\to$marks (the simulation
decouples). Each relaxation we ran opens exactly one valve and pays its
specific price: typed kicks open rate$\to$marks (count law becomes
mark-dependent; $n$ must be retuned on marked clusters), the TL-SSM gate opens
marks$\to$rate ($\ell$ no longer separates; the pin loosens and dispersion
becomes seed-dependent, \S\ref{sec:tlssm}), and a shared-state neural head
(\sspp{}, and Chang et al.'s joint training) opens both --- nothing is
certified or settable. Chang et al.\ supply the factorized \emph{form} and the
$\mathcal{L}_{\mathrm{Time}}/\mathcal{L}_{\mathrm{Set}}$ split but train both
factors jointly from one hidden state; the parametric separation, the
mark-blind autonomous ground, the moment-based setting of the time block, and
the resulting invariance guarantees are the contribution here.
...
\paragraph{TL-SSM results (SOL; all three seeds, after a calibrator fix).}
The initial run exposed a protocol fragility: $\kappa$-bisection assumes the
probe rate is a low-noise monotone function of $\kappa$, but near criticality
the probe standard error $\sim\sqrt{F\cdot R/T_{\text{total}}}$ exceeds the
$5\%$ tolerance and brackets collapse on noise (two of three seeds failed).
The repaired calibrator (converged path untouched) adds a replicate-confirmed
deep accept and, on bracket exhaustion, a pooled regression on the exact
$\kappa$-scaled Hawkes law $1/R(\kappa)= a/\kappa + c$ (linear in $1/\kappa$)
with a high-precision confirmation; the full-scale $15\%$ verification remains
the protocol gate. All three seeds then complete
($\kappa = 0.982,\ 1.046,\ 0.986$; full-scale rate residuals $-7\%$ to $+13\%$).

The three reads, on full evidence: (a) \emph{Prediction: pass.} Accuracy
$0.196$--$0.197$ (kf2 $0.194$--$0.196$, \sspp{} $0.192$), time-NLL $-3.09$ to
$-3.16$ (kf2 $-3.035$), time-KS $0.19$ (kf2 $0.228$) --- the gate restored the
time-likelihood gradient to the backbone. (b) \emph{Pin: fail as an identity}
($\kappa$ spread $[0.982, 1.046]$ vs kf2's $[1.014,1.019]$), though now
operationally robust. (c) \emph{Dispersion: fail on reliability} --- and this
overturns the single-seed read. Per-seed Fano at $\{1,\dots,50\}$\,s:
seed~1 $[18,\dots,119]$, seed~2 $[46,\dots,401]$, seed~3 $[54,\dots,1129]$
against real $[42,\dots,433]$: right on average, but a $\times 9.5$ spread at
50\,s where kf2's transplant determinism holds seed-sd to $2$--$8\%$. The
learned gate converts dispersion from a set constant into a seed lottery ---
rate-neutrality's zero-variance transplant guarantee is exactly what the gate
spends. Kurtosis improves in mean ($38$ vs kf2's $22$; real $163$) but is
seed-unstable ($13$--$61$). Net: \lgm{}-kf2 remains the shipped simulator; the
gate is the right mechanism for prediction and kurtosis but needs its variance
\emph{targeted} --- smaller $G_{\max}$, a gate log-variance penalty, or
dispersion-loss-matched gate training --- before it can be promoted.
